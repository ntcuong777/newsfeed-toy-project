...
(defn insert-image-returning-name [{:keys [tempfile filename]} {:keys [width height max-height max-width owner]}]
  (let [baos (ByteArrayOutputStream.)
        ...
        ratio (cond
                ...
                (min 1 (/ width max-width) (/ height max-height))
                
                (some? max-height)
                (min 1 (/ height max-height))
                
                (some? max-width)
                (min 1 (/ width max-width))
                
                :else
                1)
        img-scaled (if (not= 1 ratio) 
                     (let [scale (AffineTransform/getScaleInstance (double ratio) (double ratio))
                       ...
                       (.filter transform-op img (BufferedImage. (* ratio img-width) (* ratio img-height) (.getType img))))
                     img)
        _ (ImageIO/write img-scaled "png" baos)
